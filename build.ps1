properties {
    ...
    $NuGetPackageName = "PdbTasks"
    $NuGetPackDir = "$OutputDir" + "Pack"
    $NuSpecFileName = "PdbTasks.nuspec"
...
task Pack -depends Build {
    ...
    mkdir "$NuGetPackDir\tools"
    cp "$OutputDir\PdbTasks.dll" "$NuGetPackDir\tools"
    cp "$OutputDir\PdbTasks.targets" "$NuGetPackDir\tools"
